...
export default function AppCurrentVisitss1({ patCompletionData }) {
  ...
  // const [patCompletionData, setData] = useState([]);
  // useEffect(() => {
  //   axios
  //     .get('https://pms-mobitel.herokuapp.com/vendorProjectsDatabases')
  //     .then((res) => {
  //       setData(res.data.patCompletionDataForFrontEnd);
  //       // console.log(res.data.patCompletionDataForFrontEnd);
  //     })
  //     .catch((err) => {
  //       console.log(err);
  //     });
  ...
  const chartOptions = merge(BaseOptionChart(), {
    ...
    labels: ['Pass', 'Pass with Minor', 'SAR', 'Submitted', 'Pending', 'Rejected'],
  ...
  return (
    <Card>
      <CardHeader title="PAT completion" />
