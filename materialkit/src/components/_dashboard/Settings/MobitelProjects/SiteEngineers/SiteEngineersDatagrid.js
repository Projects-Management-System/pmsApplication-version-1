...
export default function UserListDatagrid() {
        ...
        <IconButton
          ...
          onClick={() => {
            ...
            if (confirmBox === true) {
              ...
              alert('Site Engineer Name Deleted successfully!');
              fetchData();
  ...
  const deleteUser = async (selectedIDs) => {
    await axiosInstance.delete(`/siteEngineersNamesList/delete/${selectedIDs}`).catch((err) => {
  ...
  const handleEditRowsModelChange = React.useCallback(
    (model) => {
      ...
      if (editedIds.length === 0) {
        const Name = editRowData.Name.value;

        const newPost = { Name };
        const confirmBox = window.confirm('Do you want to save this name ?');
        if (confirmBox === true) {
          axiosInstance.put(`/siteEngineersNamesList/Edit/${editRowData._id.value}`, newPost);
